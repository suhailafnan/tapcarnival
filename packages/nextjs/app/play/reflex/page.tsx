...
export default function ReflexRumble() {
  ...
  const { writeContractAsync: playGame } = useScaffoldWriteContract({ contractName: "TapKarnival" }); // [web:453]
  ...
  const scheduleNextFlash = () => {
    ...
    flashTimeoutRef.current = window.setTimeout(() => {
      setFlashActive(true);
      window.setTimeout(() => setFlashActive(false), reactionWindowMs);
    }, delay);
  };
  ...
  return (
    <GameShell title="Reflex Rumble">
      ...
      <div className="flex gap-4 mt-4">
        ...
        <button className="btn btn-secondary" onClick={tap} disabled={!running}>
          {flashActive ? "FLASH! Tap Now" : "Wait..."}
        </button>
      </div>
